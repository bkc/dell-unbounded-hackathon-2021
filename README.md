...
```shell
$ time env CLASSPATH=/home/bkc/src/3rdParty/pravega-client-0.9.0/\* jython import_events.py -u  tcp://192.168.198.4:9090 --scope test -i /tmp/events.json
```

Since jython takes a long time to start, the import_events module also supports the `--purge_scope` and `--purge_redis` options
